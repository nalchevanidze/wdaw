function FunctionalWave(func: (i: number) => number): string {
  return (
    "M" +
    Array.from(
      {
      ...
      (_, i) => 30 + i + " " + (30 + func(i / 40) * 40)
    )
  );
}

function rescale(vector: number[]): string {
  return "M" + vector.map((value) => 30 + value * 40).join(" ");
}
